...
# Added F alias for your new logic
...
# ERROR: Update this path to your actual local path if needed, 
# or ensure the airflow worker has access to this specific json file.
...
def clean_and_process_data(**kwargs):
    """Clean and process the DataFrame using Mean/Mode/Median strategies"""
    ti = kwargs['ti']
    ...
    try:
        ...
        print("Calculating statistics (Median, Mean, Mode)...")
        
        # Median for age
        median_age = df_no_duplicates.approxQuantile("age", [0.5], 0.0)[0]
        
        # Mean for monthly_charges
        ...
        # Mode for internet_service
        mode_internet_service = df_no_duplicates.groupBy("internet_service").count().orderBy(F.desc("count")).first()[0]

        print(f"Stats Calculated -> Age Median: {median_age}, Monthly Mean: {mean_monthly_charges}")
        ...
        # 4. Cast age to Integer
        df_processed = df_filled.withColumn("age", F.col("age").cast(IntegerType()))
        ...
        # Store the processed DataFrame path
